(function () {
  const STR = {
    vi: {
      ...
      loveCaption: "Mỗi giây cộng thêm một chút dễ thương vào câu chuyện này.",
      days: "ngày",
      hours: "giờ",
      minutes: "phút",
      seconds: "giây",
    ...
    en: {
      ...
      loveCaption: "Every second adds one more soft detail to this story.",
      days: "days",
      hours: "hours",
      minutes: "minutes",
      seconds: "seconds",
  ...
  function computeLoveParts(isoDate) {
    ...
    const startMs = window.FunFamLoveTimer.parseStartMs(isoDate);
    return window.FunFamLoveTimer.splitDurationParts(Date.now() - startMs);
  }
  ...
  function init() {
    ...
    if (project.loveSince) {
      ...
      const startMs = window.FunFamLoveTimer.parseStartMs(project.loveSince);

      loveTicker = window.FunFamLoveTimer.createTicker({
        getParts: () => window.FunFamLoveTimer.splitDurationParts(Date.now() - startMs),
        onTick: (parts, setNumber) => {
          setNumber(root.querySelector('[data-love="days"]'), String(parts.days));
          setNumber(root.querySelector('[data-love="hours"]'), String(parts.hours).padStart(2, "0"));
          setNumber(root.querySelector('[data-love="minutes"]'), String(parts.minutes).padStart(2, "0"));
          setNumber(root.querySelector('[data-love="seconds"]'), String(parts.seconds).padStart(2, "0"));
        },
      });
      loveTicker.start();
    }
